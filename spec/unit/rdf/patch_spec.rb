...
describe ActiveFedora::SparqlInsert do
  ...
  let(:expected){
  	[
  	"D <> <info:fedora/fedora-system:def/relations-external#hasConstituent> ?change .",
  	"D <> <http://purl.org/dc/terms/title> ?change .",
  	"A <> <info:fedora/fedora-system:def/relations-external#hasConstituent> <http://localhost:8983/fedora/rest/test/foo> .",
  	"A <> <http://purl.org/dc/terms/title> \"bar\" ."
  	].join("\n")
  }
  subject { ActiveFedora::RDF::Patch.new(change_set.changes) }
